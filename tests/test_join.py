...
def test_buffering():
    ...
    eq_(len(list(buffered(l, sz))), 10)

    eq_(len(list(buffered(l, sz * 2))), 5)

    eq_(len(list(buffered(l, sz * 10))), 1)
...
def test_join_key_expr():
    ...
    ctx = None
    eq_(left_key((1,), ctx), 1)

    eq_(right_key((1, 0), ctx), 1)
...
def test_join_keys():
    ...
    eq_(left_key((1,), ctx), (1, 1))

    eq_(right_key((1, 0), ctx), (1, 0))
